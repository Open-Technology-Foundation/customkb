...
# KB metadata: tuple of (short_description, full_paragraph_description)
# Used by list_knowledgebases and get_kb_info tools for discovery.
KB_METADATA = {
  'appliedanthropology': (
...
async def run_customkb_search(kb: str, query: str, top_k: int = 50, output_format: str = 'markdown') -> str:
  ...
  try:
    proc = await asyncio.create_subprocess_exec(
      *cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE, env={**os.environ, 'VECTORDBS': VECTORDBS}
    )
...
# ============================================================================
# Utility Tools — discovery and introspection of available KBs
# ============================================================================


@mcp.tool(
  name='list_knowledgebases',
  annotations={'title': 'List Available Knowledgebases', 'readOnlyHint': True, 'idempotentHint': True},
)
...
@mcp.tool(name='get_kb_info', annotations={'title': 'Get Knowledgebase Details', 'readOnlyHint': True, 'idempotentHint': True})
async def get_kb_info(knowledgebase: str = Field(..., description='Name of the knowledgebase')) -> str:
  ...
  short_desc, full_desc = KB_METADATA.get(knowledgebase, ('No description available', 'No detailed description available.'))
...
# ============================================================================
# KB-Specific Search Tools — one @mcp.tool per knowledgebase
# Each tool delegates to run_customkb_search() with a fixed KB name.
# ============================================================================


@mcp.tool(
  name='search_appliedanthropology',
  annotations={'title': 'Search Applied Anthropology KB', 'readOnlyHint': True, 'idempotentHint': True},
)
...
@mcp.tool(
  ...
  annotations={'title': 'Search Jakarta Post Archive KB', 'readOnlyHint': True, 'idempotentHint': True},
)
...
@mcp.tool(
  ...
  annotations={'title': 'Search Okusi Associates KB', 'readOnlyHint': True, 'idempotentHint': True},
)
...
@mcp.tool(name='search_okusimail', annotations={'title': 'Search Okusi Mail KB', 'readOnlyHint': True, 'idempotentHint': True})
...
@mcp.tool(
  name='search_okusiresearch', annotations={'title': 'Search Okusi Research KB', 'readOnlyHint': True, 'idempotentHint': True}
)
...
@mcp.tool(name='search_ollama', annotations={'title': 'Search Ollama KB', 'readOnlyHint': True, 'idempotentHint': True})
...
@mcp.tool(
  name='search_openai_docs', annotations={'title': 'Search OpenAI Docs KB', 'readOnlyHint': True, 'idempotentHint': True}
)
...
@mcp.tool(
  ...
  annotations={'title': 'Search Indonesian Regulations KB', 'readOnlyHint': True, 'idempotentHint': True},
)
...
@mcp.tool(
  name='search_prosocial', annotations={'title': 'Search Prosocial World KB', 'readOnlyHint': True, 'idempotentHint': True}
)
...
@mcp.tool(
  name='search_seculardharma', annotations={'title': 'Search Secular Dharma KB', 'readOnlyHint': True, 'idempotentHint': True}
)
...
@mcp.tool(name='search_smi', annotations={'title': 'Search SMI KB', 'readOnlyHint': True, 'idempotentHint': True})
...
@mcp.tool(name='search_uv', annotations={'title': 'Search UV Technical KB', 'readOnlyHint': True, 'idempotentHint': True})
async def search_uv(
...
@mcp.tool(
  name='search_wayang', annotations={'title': 'Search Wayang Culture KB', 'readOnlyHint': True, 'idempotentHint': True}
)
