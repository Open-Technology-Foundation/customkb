...
declare -- DEFAULT_SRC=''
if [[ -n "${VECTORDBS:-}" ]]; then
  DEFAULT_SRC="$VECTORDBS"/appliedanthropology/staging.text/
fi

# Flags
declare -i DRY_RUN=0 BACKUP=1 FORCE=0
...
create_frontmatter() {
  local -- title=$1 author=$2 year=$3 db_broad_context=$4
  ...
  [[ -z "$year" ]] || echo "date: \"$year\""
  [[ -z "$effective_context" ]] || echo "broad_context: \"$effective_context\""
  echo '---'
}
...
while (($#)); do 
  case $1 in
    -d|--database) 
      ...
      ;;
    -c|--context)
      noarg "$@"; shift; BROAD_CONTEXT_CLI=$1
      ;;
    -n|--dry-run) DRY_RUN=1 ;;
...
[[ -f "$DATABASE" ]] || die 2 "Database not found ${DATABASE@Q}. Run gen-citations.sh first."
...
for src_dir in "${SRC_DIRS[@]}"; do
  ...
  ((HAS_SUDO==0)) || sudo chown -R "$USER":"$USER" "${src_dir:=jUnK}"/
  ...
  for citation_line in "${citations[@]}"; do
    ...
    # Validate sourcefile path for security (prevent path traversal)
    if ! validate_file_path "$sourcefile" 2>/dev/null; then
      ERRORS+=1
      warn "Skipping invalid path ${sourcefile@Q}"
      continue
    fi
    ...
    # Additional validation: ensure resolved path stays within src_dir
    if ! validate_file_path "$file_path" "$src_dir" 2>/dev/null; then
      ERRORS+=1
      warn "Path escapes source directory ${sourcefile@Q}"
      continue
    fi
    ...
    if [[ ! -f "$file_path" ]]; then
      debug "File not found ${file_path@Q}"
      continue
    fi
    ...
    if has_frontmatter "$file_path" && ((! FORCE)); then
      SKIPPED+=1
      debug "Skipping file with existing frontmatter ${sourcefile@Q}"
      continue
    fi
    ...
    if [[ -z "$title" && -z "$author" && -z "$year" ]]; then
      ...
      if has_frontmatter "$file_path" && ((FORCE)); then
        if ((DRY_RUN)); then
          echo
          echo "Would remove frontmatter from ${sourcefile@Q} (all fields empty)"
          continue
        fi
        ...
        if ((BACKUP)); then
          cp "$file_path" "$file_path".bak || {
            ERRORS+=1
            error "Failed to create backup for ${sourcefile@Q}"
            continue
          }
        fi
        ...
        if echo "$content" > "$file_path"; then
          UPDATED+=1
          debug "Removed frontmatter from ${sourcefile@Q}"
          if ((VERBOSE > 1)); then
            echo
            success "Removed frontmatter from ${sourcefile@Q} (all citation fields empty)"
          fi
        else
          ERRORS+=1
          error "Failed to remove frontmatter from ${sourcefile@Q}"
        ...
        SKIPPED+=1
        debug "Skipping file with no citation data ${sourcefile@Q}"
      fi
      continue
    fi
    ...
    if ((DRY_RUN)); then
      echo
      echo "Would update ${sourcefile@Q}"
      [[ -z "$title" ]] || echo "  Title:      $title"
      [[ -z "$author" ]] || echo "  Author:     $author"
      [[ -z "$year" ]] || echo "  Year:       $year"
      continue
    fi
    ...
    if {
      ...
      ERRORS+=1
      error "Failed to update ${sourcefile@Q}"
