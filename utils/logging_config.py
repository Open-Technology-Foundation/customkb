#!/usr/bin/env python
"""Centralized logging for CustomKB.

Provides the logging infrastructure used across all CustomKB modules:

- **Logger factory**: ``get_logger(__name__)`` returns a module-scoped logger.
- **Setup**: ``setup_logging()`` configures the root logger with colored console
  output (via *colorlog*) and an optional rotating file handler. Configuration
  is loaded from a KB's ``.cfg`` ``[LOGGING]`` section with environment-variable
  overrides (``LOGGING_LEVEL``, ``LOGGING_FILE``, etc.).
- **Structured helpers**: ``log_operation_error``, ``log_performance_metrics``,
  ``log_file_operation``, and ``log_model_operation`` emit consistently
  formatted log lines with context and automatic unit formatting.
- **OperationLogger**: A context manager that wraps an operation with
  timing, checkpoint, and error logging.
- **Time utilities**: ``elapsed_time`` and ``time_to_finish`` for
  human-readable duration strings used in progress output.
- **Third-party silencing**: Verbose loggers (urllib3, torch, etc.) are
  set to WARNING on import to reduce noise.
"""
...
def load_logging_config(config_file: str | None = None, kb_directory: str | None = None) -> dict[str, Any]:
  ...
  if cfg.get('console_colors', True):
    con.setFormatter(
      colorlog.ColoredFormatter(
        '%(log_color)s%(module)s:%(levelname)s: %(message)s',
        reset=True,
  ...
  if log_to_file and cfg.get('file_logging', True):
    ...
    try:
      fh = logging.handlers.RotatingFileHandler(
        log_file, maxBytes=cfg.get('max_log_size', 10485760), backupCount=cfg.get('backup_count', 5), encoding='utf-8'
      )
...
def log_operation_error(logger: logging.Logger, operation: str, error: Exception, **context):
  ...
  items = [f'{k}={mask_sensitive_data(str(v)) if any(s in k.lower() for s in sensitive) else v}' for k, v in context.items()]
...
def log_file_operation(logger: logging.Logger, operation: str, file_path: str, **kwargs):
  ...
  try:
    ...
    parts = [f'{k}={v:,}bytes' if k == 'file_size' else f'{k}={v}' for k, v in extra.items() if k != 'file_path']
    ...
    logger.info(f'File {operation}: {os.path.basename(file_path)}', extra={'operation': operation, 'file_path': file_path})
...
def log_model_operation(logger: logging.Logger, operation: str, model: str, **kwargs):
  ...
  def fmt(k, v):
    return f'{k}={v:,}tokens' if k in ('token_count', 'max_tokens') else f'{k}={v:.2f}' if k == 'temperature' else f'{k}={v}'
