...
class TableNotFoundError(DatabaseError):
  ...
  def __init__(self, table_name: str):
    ...
    super().__init__(f"Table '{table_name}' not found", {'table': table_name})
...
class ModelError(APIError):
  ...
  def __init__(self, model: str, reason: str):
    """Initialize with model details."""
    super().__init__(f"Model '{model}' error: {reason}", {'model': model, 'reason': reason})
...
class DocumentProcessingError(ProcessingError):
  ...
  def __init__(self, document: str, reason: str):
    ...
    super().__init__(f"Failed to process document '{document}': {reason}", {'document': document, 'reason': reason})
...
class BatchError(ProcessingError):
  ...
  def __init__(self, batch_id: str, reason: str, failed_items: int = 0, total_items: int = 0):
    ...
    details = {'batch_id': batch_id, 'reason': reason, 'failed_items': failed_items, 'total_items': total_items}
...
class TokenLimitExceededError(ProcessingError):
  ...
  def __init__(self, tokens: int, limit: int):
    ...
    super().__init__(f'Token limit exceeded: {tokens} > {limit}', {'tokens': tokens, 'limit': limit})
...
class NoResultsError(QueryProcessingError):
  ...
  def __init__(self, query: str):
    ...
    super().__init__(f'No results found for query: {query[:100]}', {'query': query[:100]})
...
class PermissionError(FileSystemError):
  ...
  def __init__(self, filepath: str, operation: str):
    ...
    super().__init__(f'Permission denied for {operation} on {filepath}', {'filepath': filepath, 'operation': operation})
...
class InputValidationError(ValidationError):
  ...
  def __init__(self, field: str, value: Any, reason: str):
    ...
    super().__init__(f'Invalid {field}: {reason}', {'field': field, 'value': str(value)[:100], 'reason': reason})
...
class MemoryError(ResourceError):
  ...
  def __init__(self, required: int, available: int):
    ...
    super().__init__(
      f'Insufficient memory: required {required}MB, available {available}MB', {'required': required, 'available': available}
    )
