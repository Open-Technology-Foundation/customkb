...
class PerformanceAnalyzer:
  ...
  Collects and reports on memory usage, database statistics, FAISS index
  characteristics, cache hit ratios, and query latency.  Use
  ``generate_report()`` for a full text summary, or ``benchmark_query()``
  to measure query timing over multiple iterations.
  """

  def __init__(self, kb: KnowledgeBase):
    """Initialize the analyzer for a loaded KnowledgeBase.

    Args:
        kb: A fully initialized ``KnowledgeBase`` instance with an open
            database connection and valid FAISS index path.
    """
    self.kb = kb
    self.metrics = {'memory_usage': [], 'query_times': [], 'cache_stats': {}, 'db_stats': {}, 'index_stats': {}}

  def get_memory_info(self) -> dict[str, float]:
    """Get current memory usage information.

    Returns:
        Dict with keys ``rss_mb``, ``vms_mb``, ``percent``,
        ``available_mb``, and ``total_mb`` — all in megabytes except
        ``percent`` which is 0-100.
    """
  ...
  def generate_report(self) -> str:
    ...
    if db_stats.get('total_documents', 0) > 100000 and self.kb.sql_batch_size < 1000:
      report.append('  - Increase sql_batch_size for large databases')

    # Index recommendations
    if index_stats.get('num_vectors', 0) > 1000000 and index_stats.get('index_type') == 'IndexFlatL2':
