...
from query.response import get_provider_from_model_info
...
class CategoryGenerator:
  ...
  def _generate_categories_from_samples(self, samples: list[str], model_name: str = "claude-haiku-4-5") -> list[str]:
    ...
    if provider == 'anthropic':
      client = Anthropic(api_key=get_env('ANTHROPIC_API_KEY', None))
    else:
...
class AdaptiveCategorizer:
  ...
  def __init__(self, kb: KnowledgeBase, model_name: str | None = None,
    ...
    # Resolve provider from model registry
    model_info = get_canonical_model(self.model_name)
    self.provider = get_provider_from_model_info(model_info)
...
async def categorize_async(args: argparse.Namespace, kb: KnowledgeBase, logger) -> str:
  ...
  categorizer = AdaptiveCategorizer(
    kb=kb,
    model_name=getattr(args, 'model', 'claude-haiku-4-5'),
  ...
  existing_results = []
  if not (hasattr(args, 'fresh') and args.fresh):
