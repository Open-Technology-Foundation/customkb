...
def rebuild_bm25_with_filter(
  ...
  try:
    ...
    bm25_data = {
      ...
      'filters': {'keywords': keywords, 'include_patterns': include_patterns, 'exclude_patterns': exclude_patterns},
    }
  ...
  except (sqlite3.Error, OSError, pickle.PicklingError, ValueError) as e:
    logger.error(f'Error building BM25 index: {e}')
    return False


def main():
  parser = argparse.ArgumentParser(description='Rebuild BM25 index with filtering for specific content')
  parser.add_argument('config_file', help='Path to KB configuration file')
  parser.add_argument('--keywords', nargs='+', help='Keywords to filter documents (e.g., dharma secular samin ethics)')
  parser.add_argument('--include-paths', nargs='+', help='Path patterns to include (e.g., secular_dharma faqs CoT)')
  parser.add_argument('--exclude-paths', nargs='+', help='Path patterns to exclude (e.g., zenquotes test)')
  parser.add_argument('--output', help='Output path for BM25 index (defaults to KB directory)')
  parser.add_argument('--restore-backup', action='store_true', help='Restore the backup BM25 index if it exists')
