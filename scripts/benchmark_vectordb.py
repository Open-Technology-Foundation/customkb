...
def benchmark_sqlite_vec(vectors, ids, query_vectors, faiss_baseline_results):
  ...
  try:
    ...
    for start in range(0, len(vectors), batch_size):
      ...
      conn.executemany('INSERT INTO vec_items(id, embedding) VALUES (?, ?)', rows)
    conn.commit()
    insert_time = time.perf_counter() - t0
    results['insert_time_s'] = round(insert_time, 3)
    results['insert_rate'] = round(len(vectors) / insert_time, 0)
    ...
    for qv in query_vectors:
      t0 = time.perf_counter()
      rows = conn.execute(
        'SELECT id, distance FROM vec_items WHERE embedding MATCH ? ORDER BY distance LIMIT ?', (qv.tobytes(), QUERY_TOP_K)
      ).fetchall()
      query_times.append(time.perf_counter() - t0)
