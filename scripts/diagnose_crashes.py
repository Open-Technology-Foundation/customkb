...
def check_test_files():
  ...
  for test_file in test_dir.rglob('test_*.py'):
    try:
      ...
      if issues:
        problematic.append({'file': str(test_file), 'issues': issues})
    except OSError as e:
...
def check_pytest_plugins():
  ...
  try:
    result = subprocess.run(
      [sys.executable, '-m', 'pytest', '--version', '--verbose'], capture_output=True, text=True, timeout=10
    )
    print(result.stdout)
  ...
  try:
    ...
    for name, flag in limits_to_check:
      try:
        ...
        result = subprocess.run(['/bin/bash', '-c', f'ulimit {flag}'], capture_output=True, text=True, check=False)
...
def check_for_crash_logs():
  ...
  for log in log_locations:
    if log == 'dmesg':
      ...
      try:
        result = subprocess.run(['tail', '-n', '100', log], capture_output=True, text=True, timeout=5)
        if result.returncode == 0:
