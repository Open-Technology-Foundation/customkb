    ...
    for file in files:
      ...
      try:
        ...
        if expected_dims and len(embedding) != expected_dims:
          logger.warning(
            f'Dimension mismatch in {file_path}: got {len(embedding)}, expected {expected_dims} for model {model}'
          )
          corrupted_files += 1
          if remove_file(file_path):
            cleaned_files += 1
...
def get_cache_stats() -> dict[str, Any]:
  ...
  """
  stats = {'total_files': 0, 'total_size_mb': 0.0, 'subdirectories': 0, 'by_model': {}}
...
def main():
  ...
  parser = argparse.ArgumentParser(
    description='Clean corrupted embedding cache files', formatter_class=argparse.RawDescriptionHelpFormatter
  )

  parser.add_argument('--dry-run', action='store_true', help='Show what would be cleaned without removing files')
