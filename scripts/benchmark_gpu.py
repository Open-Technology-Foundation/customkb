...
def run_query(config_file, query, device='cuda'):
  ...
  if device == 'cpu':
    subprocess.run(['sed', '-i.bak', 's/reranking_device = cuda/reranking_device = cpu/', config_file], capture_output=True)

  start_time = time.time()
  result = subprocess.run(['customkb', 'query', config_file, query], capture_output=True, text=True)
...
def main():
  ...
  parser.add_argument('--query', default='What is dharma in the secular context?', help='Query to benchmark')
  parser.add_argument('--iterations', type=int, default=3, help='Number of iterations per device')
