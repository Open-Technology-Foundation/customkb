...
class TestBuiltKBStructure:
  ...
  def test_database_exists(self, test_kb):
    ...
    assert os.path.exists(test_kb['db_path']), f'Database not found: {test_kb["db_path"]}'

  def test_faiss_index_exists(self, test_kb):
    """FAISS index file should be created by process_embeddings."""
    assert os.path.exists(test_kb['faiss_path']), f'FAISS index not found: {test_kb["faiss_path"]}'
  ...
  def test_database_has_chunks(self, test_kb):
    ...
    try:
      ...
      assert source_count >= 50, f'Expected chunks from >= 50 source files, got {source_count}'
    finally:
      conn.close()
  ...
  def test_all_chunks_embedded(self, test_kb):
    ...
    try:
      ...
      assert unembedded == 0, f'{unembedded} chunks still have embedded=0'
    finally:
      conn.close()
  ...
  def test_faiss_dimensions(self, test_kb):
    ...
    assert index.d == 384, f'Expected 384 dimensions, got {index.d}'
  ...
  def test_faiss_has_vectors(self, test_kb):
    ...
    try:
      ...
      assert index.ntotal == unique_count, f'FAISS has {index.ntotal} vectors but DB has {unique_count} unique embedded chunks'
    finally:
      conn.close()

  def test_bm25_index_exists(self, test_kb):
    """BM25 index (.bm25.npz) should be created by build_bm25_index."""
    bm25_path = os.path.join(test_kb['kb_dir'], f'{test_kb["kb_name"]}.bm25.npz')
    assert os.path.exists(bm25_path), f'BM25 index not found: {bm25_path}'
...
class TestCitations:
  ...
  def test_citations_have_titles(self, test_kb):
    ...
    try:
      cursor = conn.cursor()
      cursor.execute("SELECT COUNT(*) FROM citations WHERE title IS NOT NULL AND title != '' AND title != 'NF'")
