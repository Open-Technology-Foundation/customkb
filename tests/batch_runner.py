...
class MemoryMonitor:
  ...
  def get_summary(self) -> str:
    ...
    return f'Memory: {current["rss_mb"]:.1f}MB (+{delta:.1f}MB from start, {current["percent"]:.1f}% of system)'


def run_test_batch(batch_name: str, batch_config: dict, verbose: bool = False, coverage: bool = False) -> dict[str, any]:
...
def main():
  ...
  parser = argparse.ArgumentParser(description='Run CustomKB tests in safe batches with memory limits')
  parser.add_argument('--batch', choices=list(TEST_BATCHES.keys()) + ['all'], help='Batch to run (or "all" for all batches)')
  parser.add_argument('--list', action='store_true', help='List available batches')
  parser.add_argument('--memory-limit', type=float, help='Override memory limit in GB')
  parser.add_argument('--verbose', '-v', action='store_true', help='Verbose output')
  parser.add_argument('--coverage', action='store_true', help='Generate coverage report')
  parser.add_argument('--stop-on-failure', action='store_true', help='Stop running batches after first failure')
  parser.add_argument('--force', action='store_true', help='Force run even if system memory is high')
  ...
  if args.memory_limit:
    for batch_config in TEST_BATCHES.values():
      batch_config['memory_limit_gb'] = min(args.memory_limit, batch_config['memory_limit_gb'])
  ...
  if args.coverage and overall_success:
    ...
    subprocess.run([sys.executable, '-m', 'coverage', 'report', '--skip-covered', '--skip-empty'])
