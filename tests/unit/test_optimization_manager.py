...
class TestOptimizationManager:
  ...
  def test_get_system_memory_gb_fallback(self):
    ...
    with patch('psutil.virtual_memory', side_effect=ImportError), patch('builtins.open', mock_open(read_data=meminfo_content)):
      result = get_system_memory_gb()
      assert result == 16.0
  ...
  def test_process_optimize_target_resolution(self):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
      ...
      args = Mock(show_tiers=False, target=config_path, dry_run=True, analyze=False, memory_gb=None)
      logger = Mock()
...
class TestIndexCreation:
  ...
  def test_optimize_creates_indexes(self, mock_create_indexes, mock_kb):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
      ...
      # Run optimization
      optimize_config(config_path, dry_run=False, check_indexes=True)
