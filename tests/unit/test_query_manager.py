...
class TestQueryCaching:
  ...
  def test_get_cached_query_embedding_hit(self, temp_data_manager):
    ...
    with (
      patch('query.embedding.QUERY_CACHE_DIR', temp_dir),
      patch('query.embedding.get_cache_key', return_value=cache_key),
      patch('time.time', return_value=1000),
      patch('os.path.getmtime', return_value=999),
    ):  # Recent file
  ...
  def test_save_query_embedding_to_cache(self, temp_data_manager):
    ...
    with patch('query.embedding.QUERY_CACHE_DIR', temp_dir), patch('query.embedding.get_cache_key', return_value='test_key'):
...
class TestQueryEmbedding:
  ...
  async def test_get_query_embedding_cached(self):
    ...
    with (
      patch('query.embedding.get_cached_query_embedding', return_value=test_embedding),
  ...
  async def test_get_query_embedding_api_call(self):
    ...
    # Patch at the actual implementation module
    with (
      patch('query.embedding.get_cached_query_embedding', return_value=None),
...
class TestContextFileReading:
  ...
  def test_read_context_file_permission_error(self, temp_data_manager):
    ...
    with (
      patch('query.processing.read_text_file', side_effect=PermissionError('Access denied')),
...
class TestReferenceStringBuilding:
  ...
  def test_build_reference_string_with_context_files(self, temp_config_file):
    ...
    context_files_content = [('Context file content', 'context'), ('Additional context', 'extra')]

    reference = [[1, 'test.txt', 0, 'Main content', 0.8, '{}']]
  ...
  def test_build_reference_string_metadata_parsing(self, temp_config_file):
    ...
    metadata = {'heading': 'Test Heading', 'section_type': 'code_block', 'char_length': 100, 'word_count': 20}

    # Reference structure: [rid, rsrc, rsid, originaltext, distance, metadata, primary_category, categories]
    reference = [[1, 'test.txt', 0, 'Content with metadata', 0.8, json.dumps(metadata), None, None]]
  ...
  def test_build_reference_string_xml_escaping(self, temp_config_file):
    ...
    reference = [[1, 'test.txt', 0, 'Content with <tags> & special chars', 0.8, '{}']]
  ...
  def test_build_reference_string_invalid_metadata(self, temp_config_file):
    ...
    reference = [[1, 'test.txt', 0, 'Content', 0.8, 'invalid json metadata']]
...
class TestOpenAIResponsesAPIHelpers:
  ...
  def test_extract_content_from_response(self):
    ...
    response_data_alt = {'output': [{'type': 'message', 'content': [{'text': 'Alternative text'}]}]}
...
class TestAIResponseGeneration:
  ...
  async def test_generate_ai_response_openai(self, temp_config_file):
    ...
    with (
      patch('query.llm.get_canonical_model', return_value=mock_model_info),
  ...
  async def test_generate_ai_response_grok_no_api_key(self, temp_config_file):
    ...
    with (
      ...
      patch(
        'query.llm.litellm.acompletion',
        new_callable=AsyncMock,
        side_effect=litellm.AuthenticationError(message='No API key', model='xai/grok-4', llm_provider='xai'),
      ),
...
class TestProcessQuery:
  ...
  async def test_process_query_async_context_only(self, temp_database, temp_config_file, mock_faiss_index, mock_db_connection):
    ...
    with (
      patch('query.processing.KnowledgeBase', return_value=kb),
  ...
  async def test_process_query_with_query_file(self, temp_config_file, temp_data_manager, mock_db_connection):
    ...
    with (
      patch('query.processing.get_fq_cfg_filename', return_value=temp_config_file),
      ...
      with (
        ...
        mock_refs = [
          (0, 'doc1.txt', 1, 'Sample text 1', 0.9, {'similarity': 0.9}),
