...
class TestGetCanonicalModel:
  ...
  def test_direct_model_lookup(self, temp_data_manager):
    ...
    models_data = {'gpt-4': {'model': 'gpt-4', 'provider': 'openai', 'type': 'chat'}}
  ...
  def test_alias_lookup(self, temp_data_manager):
    ...
    models_data = {
      'gpt-4-turbo-preview': {'model': 'gpt-4-turbo-preview', 'alias': 'gpt4', 'provider': 'openai', 'type': 'chat'}
    }
  ...
  def test_partial_match_fallback(self, temp_data_manager):
    ...
    models_data = {'text-embedding-3-large': {'model': 'text-embedding-3-large', 'provider': 'openai', 'type': 'embedding'}}
  ...
  def test_model_not_found(self, temp_data_manager):
    ...
    models_data = {'gpt-4': {'model': 'gpt-4', 'provider': 'openai'}}

    models_file = self._create_models_file(temp_data_manager, models_data)

    with patch('models.model_manager.models_file', models_file), pytest.raises(KeyError, match='nonexistent-model not found'):
      get_canonical_model('nonexistent-model')

  def test_models_file_not_found(self):
    """Test behavior when Models.json file is not found."""
    with (
      patch('models.model_manager.models_file', '/nonexistent/Models.json'),
  ...
  def test_multiple_partial_matches(self, temp_data_manager):
    ...
    models_data = {
      'text-embedding-ada-002': {'model': 'text-embedding-ada-002', 'provider': 'openai', 'type': 'embedding'},
      'text-embedding-3-small': {'model': 'text-embedding-3-small', 'provider': 'openai', 'type': 'embedding'},
      'text-embedding-3-large': {'model': 'text-embedding-3-large', 'provider': 'openai', 'type': 'embedding'},
    }
...
class TestModelDataValidation:
  ...
  def test_valid_openai_model(self, temp_data_manager):
    ...
    models_data = {
      'gpt-4': {'model': 'gpt-4', 'provider': 'openai', 'type': 'chat', 'max_tokens': 8192, 'supports_functions': True}
    }
...
class TestLoggingIntegration:
  ...
  def test_partial_match_warning_logged(self, temp_data_manager):
    ...
    models_data = {'gpt-4-turbo-preview': {'model': 'gpt-4-turbo-preview', 'provider': 'openai'}}
